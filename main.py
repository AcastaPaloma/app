...
def home():
   ...
   #-----------------------------------------------------------------------------------------------------------
    ...
    questionmarks3 = '?' * len(tupled_wanted_ids)
...
def search_results():
    ...
    connection = sqlite3.connect('database2.sqlite')
    cur = connection.cursor()
    cur.execute("SELECT * FROM Artpieces WHERE name Like ? OR artist_name Like ? OR image_name Like ? OR artpiece_type Like ?", (requested_search,requested_search, requested_search, requested_search))
...
def process():
    ...
    connection = sqlite3.connect('database2.sqlite')
    cur = connection.cursor()
    cur.execute("INSERT INTO Artpieces(name, artist_name, image_name, email, message, image_filename, artpiece_type) VALUES (?, ?, ?, ?, ?, ?, ?)", (username, artist_name, title, email, message, my_filename, type))
    connection.commit()
    connection.close()
...
def secret_upload():
    ...
    cur.execute(
        ...
        (requested_server_name, requested_server_charact, requested_link_to_server, my_icon_filename, requested_server_description))
    connection.commit()
    connection.close()
